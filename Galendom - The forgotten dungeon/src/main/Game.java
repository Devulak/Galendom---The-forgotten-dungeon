...
public class Game implements GameInterface
{
	...
	/**
	 * Increments the number of turns by one everytime the player moves around
	 * the map. The player will be warned that he will lose health if the amount
	 * of turns equals the turn limit. The player will gradually lose health if
	 * he stays in the dungeon for long.
	 */
	@Override
	public void addTurn()
	{
	...
	/**
	 * Will search through your inventory. If you wanted to use potion, this
	 * method will find a class called Potion. Otherwise it will return false.
	 *
	 * @param searchForItem
	 * @return true, false
	 */
	...
	public void goRoom(int[] direction)
	{
		...
		else
		{
			...
			int[] newPosition =
			{
				getPlayerPosition()[0] + direction[0], getPlayerPosition()[1] + direction[1]
			};
	...
	/**
	 * Makes the vendor walk "again", vendor cannot move outside a locked room,
	 * nor can he move into locked rooms. The vendor travels at random exits, so
	 * he might go forward and back between two of the same rooms, but that's
	 * okay, he may be a bit silly but he's our silly!
	 */
	...
	{
		return currentRoom == currentVendorRoom;
	}

	@Override
	public boolean roomHasTeleporter()
	{
		return currentRoom.getTeleporter() != null;
	}
